...
import { fetchAllEventbriteEvents } from '@/lib/eventbrite'
...
export async function GET(request: Request) {
  ...
  try {
    const events = await fetchAllEventbriteEvents(start, end)
    if (!save) {
      return NextResponse.json({ count: events.length, events })
    }
    const { saved, skipped } = await upsertEvents(events)
    return NextResponse.json({ message: `Eventbrite: fetched ${events.length}, saved ${saved}, skipped ${skipped}`, saved, skipped, total: events.length })
  } catch (error) {
